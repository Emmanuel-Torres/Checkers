...
const hasValidMoves = (row: number, column: number, board: Piece[][], isRoomOwner: boolean): boolean => {
...
const getValidMoves = (row: number, colum: number, board: Piece[][], isRoomOwner: boolean): number[][] => {
    return [];
}

const hasAvailableNeighbor = (row: number, column: number, board: Piece[][], rowDirection: number): boolean => {
    const piece = board[row][column];
    for(let i = 0; i < 2; i++) {
